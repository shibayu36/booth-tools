...
async function getOrders(browser, page) {
  ...
  let orderLinks = await page.evaluate(() => {
    return Array.from(document.querySelectorAll("a.link-ext")).map(
      (a) => a.href
    );
  });
  ...
  for (let link of orderLinks) {
    ...
    let row = await orderPage.evaluate(() => {
      ...
      let name = panel
        .querySelector(".manage-order-shipment-summaries .u-tpg-slight-body")
        .textContent.match(/(.+)\s+様/)[1];
      let productContainers = panel.querySelectorAll(".manage-order-content");
      let products = Array.from(productContainers).map(
        (c) =>
          c.querySelector(".u-tpg-slight-body a.nav-reverse").innerText.trim() +
          " : " +
          c.querySelector(".u-text-right b").innerText
      );
